function VVCF=calc_vel_corr_new(alpha,delta_range,plot_range)
%
% Thomas Lampo, Andrew Kennard, and Andrew Spakowitz
% November 2015
%  edit 20240624 Bugs in interpolation fixed by Leonid Andronov
    ...
    if alpha==1
        ...
        for j=1:length(delta_range)
            ...
            if d1==d2
                ...
                prop=(d_true-d1)/(d2-d1);
                vvcf_1=prop.*vacf_1b+(1-prop).*vacf_1a; %LA edit, was vvcf_1=prop.*vacf_1a+(1-prop).*vacf_1b;
                vvcf_2=prop.*vacf_2b+(1-prop).*vacf_2a; %LA edit, was vvcf_2=prop.*vacf_2a+(1-prop).*vacf_2b;        
            end
            if alpha_1==alpha_2
                vvcf=vvcf_1;
                VVCF{j}=vvcf;
                ...
                vvcf=prop2.*vvcf_2+(1-prop2).*vvcf_1; %LA edit, was vvcf=prop2.*vvcf_1+(1-prop2).*vvcf_2;
                VVCF{j}=vvcf;
            end
        end
    end
