...
% d=600;
% col=1;
%   Dfin=evalin('base','Dfin_nonrep(1:d,col)'); % rep FOS-P Cy5; non-rep FOS-E Cy3
%   Dfiny=evalin('base','Dfiny_nonrep(1:d,col)');
%   Dfinz=evalin('base','Dfinz_nonrep(1:d,col)');
...
cvtz=zeros(CutoffLen/2,CutoffLen-1);
...
cvttempz=zeros(CutoffLen/2,CutoffLen-1);
...
cvtfinalz=zeros(CutoffLen/2+1,CutoffLen/2);
for i=1:CutoffLen/2
    for j=1:CutoffLen/2
