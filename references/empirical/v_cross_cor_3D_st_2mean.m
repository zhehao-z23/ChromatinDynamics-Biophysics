...
cvtz_G=zeros(deltarange,CutoffLen-1);
 ...
 for I=0:deltarange % possible ways to have lag time
   for d=1:deltarange    %  calculate velocity delta
    ...
    cvt_2mean(d,I+1)=mean(cv,"all","omitnan");
    cvtx_2mean(d,I+1)=mean(cvx,"all","omitnan");
    cvty_2mean(d,I+1)=mean(cvy,"all","omitnan");
    cvtz_2mean(d,I+1)=mean(cvz,"all","omitnan");
    cvt_R_2mean(d,I+1)=mean(cv_R,"all","omitnan");
    cvtx_R_2mean(d,I+1)=mean(cvx_R,"all","omitnan");
    cvty_R_2mean(d,I+1)=mean(cvy_R,"all","omitnan");
    cvtz_R_2mean(d,I+1)=mean(cvz_R,"all","omitnan");
...
cvttemp_R_2mean=zeros(deltarange,CutoffLen-1);
cvttempx_R_2mean=zeros(deltarange,CutoffLen-1);
cvttempy_R_2mean=zeros(deltarange,CutoffLen-1);
cvttempz_R_2mean=zeros(deltarange,CutoffLen-1);

cvttemp_2mean=zeros(deltarange,CutoffLen-1);
cvttempx_2mean=zeros(deltarange,CutoffLen-1);
cvttempy_2mean=zeros(deltarange,CutoffLen-1);
cvttempz_2mean=zeros(deltarange,CutoffLen-1);
...
for j=1:deltarange
    for i=1:deltarange
     ...
     cvttemp_R_2mean(i,j)=cvt_R_2mean(i,j+1)/cvt_R_2mean(i,1);  % auto-correlation
     cvttempx_R_2mean(i,j)=cvtx_R_2mean(i,j+1)/cvtx_R_2mean(i,1);
     cvttempy_R_2mean(i,j)=cvty_R_2mean(i,j+1)/cvty_R_2mean(i,1);
     cvttempz_R_2mean(i,j)=cvtz_R_2mean(i,j+1)/cvtz_R_2mean(i,1);
     cvttemp_2mean(i,j)=cvt_2mean(i,j)/cvt_R_2mean(i,1);  % cross-correlation 
     cvttempx_2mean(i,j)=cvtx_2mean(i,j)/cvtx_R_2mean(i,1);
     cvttempy_2mean(i,j)=cvty_2mean(i,j)/cvty_R_2mean(i,1);
     cvttempz_2mean(i,j)=cvtz_2mean(i,j)/cvtz_R_2mean(i,1);
    end
end
...
cvtfinalx_GR_2mean=zeros(deltarange+1,deltarange);
cvtfinal_GR_2mean=zeros(deltarange+1,deltarange);
cvtfinaly_GR_2mean=zeros(deltarange+1,deltarange);
cvtfinalz_GR_2mean=zeros(deltarange+1,deltarange);
cvtfinalx_R_2mean=zeros(deltarange+1,deltarange);
cvtfinal_R_2mean=zeros(deltarange+1,deltarange);
cvtfinaly_R_2mean=zeros(deltarange+1,deltarange);
cvtfinalz_R_2mean=zeros(deltarange+1,deltarange);
...
for i=1:deltarange
    for j=1:deltarange
      ...
      cvtfinal_R_2mean(i+1,j)=cvttemp_R_2mean(j,i);
      cvtfinalx_R_2mean(i+1,j)=cvttempx_R_2mean(j,i);
      cvtfinaly_R_2mean(i+1,j)=cvttempy_R_2mean(j,i);
      cvtfinalz_R_2mean(i+1,j)=cvttempz_R_2mean(j,i);

      cvtfinal_R_2mean(1,j)=1;
      cvtfinalx_R_2mean(1,j)=1;
      cvtfinaly_R_2mean(1,j)=1;
      cvtfinalz_R_2mean(1,j)=1;
      cvtfinal_GR_2mean(i,j)=cvttemp_2mean(j,i);
      cvtfinalx_GR_2mean(i,j)=cvttempx_2mean(j,i);
      cvtfinaly_GR_2mean(i,j)=cvttempy_2mean(j,i);
      cvtfinalz_GR_2mean(i,j)=cvttempz_2mean(j,i);
    end
end
% %%
...
% assignin('base','cvtfinalz_R',cvtfinalz_R)

assignin('base','cvtfinal_GR',cvtfinal_GR)
assignin('base','cvtfinal_GR_2mean',cvtfinal_GR_2mean)
% assignin('base','cvtfinalx_GR',cvtfinalx_GR)
% assignin('base','cvtfinaly_GR',cvtfinaly_GR)
% assignin('base','cvtfinalz_GR',cvtfinalz_GR)
...
curve=[1 5 10 50 100];
labelt=curve*FrameT;
plot(matrix(:,curve), cvtfinal_GR_2mean(:,curve),LineWidth=4); %normally, no need to loop
 % colororder (turbo(size(cvtfinal_GR,2)/2));
 % colormap (turbo(size(cvtfinal_GR,2)/2)); 
 %  colororder (turbo(trajsize));
 % colormap (turbo(trajsize)); 
